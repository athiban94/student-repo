...
class Repository:
    ...
    def __init__(self, directory, pt):
        ...
        self.db_path = "/Users/athibanp/athiban/ms-cs/SSW-810/Assignments/810_hw11.db"
        self.ins_data_db = list()
    ...
    def instructor_table_db(self):
        """ function to print instructor table from the database  """
        ...
        for cwid, name, dept, course, students in db.execute(query):
            self.ins_data_db.append((cwid, name, dept, course, students))
            row = [cwid, name, dept, course, students]
            pt.add_row(row)
        if(self.pt):
            print(pt)
...
def main():
    stevens = Repository("../HW10_Athiban_Parthiban/data", True)
